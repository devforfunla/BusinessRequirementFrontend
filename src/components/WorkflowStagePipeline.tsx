...
import { CheckCircle2, ClipboardCheck, GitBranch, ListChecks } from 'lucide-react'
...
export function WorkflowStagePipeline({ workflowId, activeStage }: { workflowId: string; activeStage: WorkflowStage }) {
  return (
    <nav className="rounded-lg border border-[#d8dee8] bg-white px-3 py-3 shadow-sm" aria-label="Workflow stages">
          ...
          return (
            <li key={stage.id} className="min-w-0">
              <Link
                ...
                className={cn(
                  'group flex min-h-14 items-center gap-3 rounded-md border px-3 py-2 transition',
                  isActive
                    ? 'border-[#1f6feb] bg-[#e8f1ff] text-[#175cd3]'
