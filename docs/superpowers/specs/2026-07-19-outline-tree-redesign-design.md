...
- Polling: `refetchInterval` on `outlineQuery` — if tree is non-empty and any node has `summary: null`, poll 5s; else `false`
...
- When summary is already populated: no summary line shown (avoid clutter)
...
6. Hit `/kb-api/documents/999999/outline` — 404 doesn't crash the page
